...
public abstract class MicroService implements Runnable {
    private HashMap<Class<?>, Callback<?>> typeCallback; //each type of event with the appropriate callback function
    ...
    public MicroService(String name) {
        typeCallback = new HashMap<>();
        terminated = false;
        theBus = MessageBusImpl.getInstance();
        this.name = name;
    }
    /**
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        ...
        typeCallback.put(type, callback);
    }
    /**
        ...
        if(!(this instanceof TimeService)) {
            while (!terminated) {
                try {
                    ...
                    Callback callFunc = typeCallback.get(msg.getClass());
                    callFunc.call(msg);
